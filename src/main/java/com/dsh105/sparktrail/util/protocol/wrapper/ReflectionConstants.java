...
public enum ReflectionConstants {
      ...
      ReflectionConstants(String[] vanilla, String... mcpc) {
            ...
            for (int i = 0; i < knownVersions.length; i++) {
                  if (Integer.valueOf("v1_7_R4".replaceAll("[^0-9]", "")) == knownVersions[i]) {
                        versionIndex = i;
                  }
            }

            for (int i = 0; i < knownMCPCVersions.length; i++) {
                  if (String.valueOf(Integer.valueOf("v1_7_R4".replaceAll("[^0-9]", ""))).startsWith(String.valueOf(knownMCPCVersions[i]))) {
                        mcpcVersionIndex = i;
                  }
            }
      }
